 ...
 * @Description:
 * @Author: xianggu
 * @CreateDate: 2019/2/16 11:49 AM
 */
public class RotateItemTouchHelperCallback <T> extends ItemTouchHelper.Callback {
    ...
    private RotateOnSlideListener<T> mListener;

    public RotateItemTouchHelperCallback(@NonNull RecyclerView.Adapter adapter, @NonNull List<T> dataList) {
        this.adapter = checkIsNull(adapter);
        this.dataList = checkIsNull(dataList);
    }

    public RotateItemTouchHelperCallback(@NonNull RecyclerView.Adapter adapter, @NonNull List<T> dataList, RotateOnSlideListener<T> listener) {
        this.adapter = checkIsNull(adapter);
        this.dataList = checkIsNull(dataList);
    ...
    public void setOnSlideListener(RotateOnSlideListener<T> mListener) {
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (layoutManager instanceof RotateLayoutManager) {
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (mListener != null) {
            mListener.onSlided(viewHolder, remove, direction == ItemTouchHelper.LEFT ? SLIDED_LEFT : SLIDED_RIGHT);
        }
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (mListener != null) {
                if (ratio != 0) {
                    mListener.onSliding(viewHolder, ratio, ratio < 0 ? SLIDING_LEFT : SLIDING_RIGHT);
                } else {
                    mListener.onSliding(viewHolder, ratio, SLIDING_NONE);
                }
            }
        }
    }
