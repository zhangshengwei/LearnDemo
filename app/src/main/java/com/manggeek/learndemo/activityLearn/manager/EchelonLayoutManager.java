...
import com.manggeek.learndemo.Entity.EchelonItemModel;

import java.util.ArrayList;

/**
 * @Description:卡片组梯形向下滑动切换卡片
 * @Author: xianggu
 * @CreateDate: 2019/2/15 3:40 PM
 */
public class EchelonLayoutManager extends RecyclerView.LayoutManager {
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        mScrollOffset = Math.min(Math.max(mItemViewHeight, mScrollOffset + dy),
    ...
    private void layoutChild(RecyclerView.Recycler recycler) {
        ...
        ArrayList<EchelonItemModel> layoutInfos = new ArrayList<>();
        for (int i = bottomItemPosition - 1,j = 1; j>=0 ; i--,j++ ){
            ...
            EchelonItemModel info = new EchelonItemModel(start, scaleXY, positonOffset, layoutPercent);
        ...
        if (bottomItemPosition < mItemCount) {
            ...
            layoutInfos.add(new EchelonItemModel(start, 1.0f, bottomItemVisibleHeight * 1.0f / mItemViewHeight, start * 1.0f / getVerticalSpace())
                    .setIsBottom());
        } else {
        ...
        for (int i = 0; i < layoutCount; i++) {
            ...
            EchelonItemModel layoutInfo = layoutInfos.get(i);
            addView(view);
